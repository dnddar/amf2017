(function($){
    $.fn.uploader = function(action, success, fail, obj){
	...
	this.find('form:eq(0)').append('<input type=hidden name=call value='+call_name+'><input type=hidden name=fail value='+fail_name+'><input type=hidden name=obj value='+obj+'>');
	this.append('<iframe name=uploader_target_'+rnd+' scrolling=no frameborder=0 style=width:1px;height:1px></iframe>');
	window[call_name] = function(filename, obj){
	    success(filename, obj);
	};
